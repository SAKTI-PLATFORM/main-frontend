...
import { DashboardError, DashboardLoading } from '@/components/dashboard/dashboard-status';
...
function GreetingBlock({ profile }: { profile: Profile }) {
  const summary = profile.profileSummary?.trim() || 'Lengkapi profil dan onboarding agar SAKTI AI dapat menyusun ringkasan karier yang lebih akurat.';
  const shortSummary = summary.length > 220 ? `${summary.slice(0, 220).trimEnd()}…` : summary;

  return (
    <section>
      <div className="flex items-start justify-between gap-4">
        <h1 className="font-heading text-[22px] leading-[35px] font-bold tracking-[-0.01em] text-[#1F1F27] sm:text-[24px] sm:leading-[30px]">
          <span className="font-medium text-[#5C5C6A]">Hai,</span> {profile.fullName}!
        </h1>
      ...
      <div className="relative mt-2 max-w-2xl">
        <p className="line-clamp-2 text-sm leading-6 text-[#6C6C7A]">
          {shortSummary}
        </p>
      </div>
    </section>
...
function IconSquare({ children, label, href, external, disabled }: { children: React.ReactNode; label: string; href?: string; external?: boolean; disabled?: boolean }) {
  const classes = 'flex size-9 items-center justify-center bg-white text-[#5B5B6E] transition-colors hover:bg-[#F4F3FB] hover:text-[#4138D8] disabled:pointer-events-none disabled:opacity-40';

  if (href && !disabled) {
    return (
      <Link href={href} aria-label={label} title={label} className={classes} {...(external ? { target: '_blank', rel: 'noreferrer' } : {})}>
        {children}
      </Link>
    );
  }
  ...
  return (
    <button type="button" aria-label={label} title={label} disabled={disabled} className={classes}>
      {children}
    </button>
  );
}
...
function LinkedinBadge({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden className={className}>
      ...
      <path fill="#fff" d="M6.7 10.4h2.4V18H6.7v-7.6Zm4.15 0h2.3v1.04h.03c.32-.6 1.1-1.24 2.27-1.24 2.43 0 2.88 1.6 2.88 3.68V18h-2.4v-3.65c0-.87-.02-1.99-1.21-1.99-1.22 0-1.4.95-1.4 1.93V18h-2.4v-7.6Z" />
    </svg>
  );
}
...
function WeeklyLearningCard({ learningPath }: { learningPath: LearningPathRun }) {
  ...
  const steps = result ? [...result.learning_path_steps].sort((left, right) => left.step_order - right.step_order) : [];
  const activeStep = steps[0] ?? null;

  if (!result || !path || !activeStep) {
    return (
      <section className="relative rounded-lg overflow-hidden bg-gradient-to-br from-[#5003FF] to-[#4714BC] p-4 text-white sm:p-5">
        <Image src="/logo-dash.png" alt="" width={265} height={276} className="pointer-events-none absolute -right-6 -top-10 w-1/2 max-w-none select-none" />
        <div className="relative">
          <p className="text-xs font-medium text-white/70">Tingkatkan Kemampuanmu</p>
          <h2 className="font-heading mt-1 text-[20px] leading-[30px] font-bold tracking-[-0.01em] sm:text-[24px] sm:leading-[36px]">{busy ? 'Roadmap belajarmu sedang disusun' : 'Roadmap belajarmu belum dibuat'}</h2>
          <p className="mt-2 max-w-xl text-[13px] leading-6 text-white/80">
            {busy ? 'TalentForger sedang menyiapkan urutan skill, durasi, dan materi belajar. Halaman ini akan diperbarui otomatis.' : 'Buat roadmap dari skill gap role incaranmu untuk memantau progres belajar mingguanmu di sini.'}
          </p>
          <Link href="/job-seeker/learning-paths" className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-white px-3.5 py-2 text-[13px] font-semibold text-[#3F27B8] transition hover:bg-white/90">
  ...
  const totalWeeks = path.estimated_duration_weeks || Math.max(...steps.map((step) => step.week), 1);
  const currentWeek = activeStep.week || 1;
  const progress = Math.min(100, Math.round((currentWeek / totalWeeks) * 100));

  const stepResourceIds = new Set(result.resource_recommendations.filter((rec) => rec.step_id === activeStep.step_id).map((rec) => rec.resource_id));
  const stepResources = [...result.learning_resources, ...result.free_materials].filter((resource) => stepResourceIds.has(resource.resource_id));
  const moduleCount = stepResources.length;
  const hours = Math.round(stepResources.reduce((sum, resource) => sum + (resource.estimated_duration_hours || 0), 0));
  ...
  return (
    <section className="relative rounded-lg overflow-hidden bg-gradient-to-br from-[#5003FF] to-[#4714BC] p-4 text-white sm:p-5">
      ...
      <div className="relative">
        ...
        <p className="mt-2 max-w-xl text-[13px] text-justify leading-4 text-white/80">
        ...
        <div className="mt-2 flex items-center justify-between text-xs text-white/75">
          <span>
            Minggu {currentWeek} dari {totalWeeks} · {activeStep.related_skill_name}
          </span>
        ...
        <div className="mt-3 flex flex-wrap gap-2.5">
          <Link href="/job-seeker/learning-paths" className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3.5 py-2 text-[13px] font-semibold text-[#3F27B8] transition hover:bg-white/90">
            <Play className="size-3.5 fill-current" />
            Lanjutkan Belajar
          </Link>
          <Link href="/job-seeker/learning-paths" className="inline-flex items-center gap-1.5 rounded-lg bg-white/10 px-3.5 py-2 text-[13px] font-medium text-white ring-1 ring-inset ring-white/25 transition hover:bg-white/20">
...
function SkillGapCard({ jobMatcher }: { jobMatcher: JobMatcherRun }) {
  ...
  const sortedGaps = [...gaps].sort((left, right) => priorityRank(left.priority) - priorityRank(right.priority));
  const visibleGaps = sortedGaps.slice(0, 6);
  const busy = jobMatcher?.status === 'PENDING' || jobMatcher?.status === 'RUNNING';
    ...
    <section className="bg-white p-4 sm:p-5">
      ...
      {gaps.length > 0 ? (
        <>
          <div className="relative mt-3">
            ...
            <div className="flex items-end gap-1.5">
              {visibleGaps.map((gap, index) => {
                ...
                const isLast = index === visibleGaps.length - 1;
                return (
                  <div key={`${gap.gap_id}-${index}`} className="group relative flex min-w-0 flex-1 flex-col items-center gap-1.5">
                    <div
                      className={cn(
                    ...
                    <div className="flex h-[90px] w-full items-end justify-center">
                      <div className="relative w-full max-w-[88px] overflow-hidden rounded-sm bg-[#E7E2FA] transition-colors group-hover:bg-[#DCD4F8]" style={{ height: `${Math.max(required, 14)}%` }}>
                        <div className="absolute inset-x-0 bottom-0 rounded-sm bg-[#5B3FE4]" style={{ height: `${darkFraction * 100}%` }} />
                      </div>
                    </div>
          ...
          <div className="mt-3 flex items-end justify-between gap-4 border-t border-[#F0F0F5] pt-3">
            ...
            <Link href="/job-seeker/job-matches" className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-[#E4E3F0] px-3 py-2 text-xs font-semibold text-[#4138D8] transition hover:border-[#CFC8FF] hover:bg-[#F6F5FF]">
              Lihat Analisis
        ...
        <div className="mt-4 flex flex-col items-center justify-center rounded-lg bg-[#FAFAFC] px-6 py-6 text-center">
          ...
          <p className="mt-3 text-sm font-semibold text-[#2F2F3A]">{busy ? 'Analisis skill gap sedang diproses' : 'Belum ada analisis skill gap'}</p>
          <p className="mt-1 max-w-sm text-xs leading-5 text-[#9A9AAB]">
            {busy ? 'JobMatcher sedang membandingkan skill-mu dengan kebutuhan role. Hasil muncul otomatis.' : 'Jalankan Job Matches untuk melihat gap skill terhadap role rekomendasimu.'}
          </p>
          <Link href="/job-seeker/job-matches" className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-[#3E1DD1] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#3315B8]">
            Buka Job Matches
            <ArrowUpRight className="size-3.5" />
          </Link>
        </div>
  ...
  return (
    <div className="overflow-hidden rounded-lg bg-white ring-1 ring-[#ECECF2]"> 
      <AccordionRow
        ...
        subtitle={confidence != null ? `Big Five · Confidence ${Math.round(confidence)}%` : 'Big Five'}
      ...
      >
        {assessment?.ocean ? (
          <>
            ...
            />
            <p className="mt-3 text-sm leading-6 text-[#5C5C6A]">{assessment.career_profile_summary}</p>
          </>
        ) : (
          <EmptyText>Selesaikan asesmen OCEAN &amp; RIASEC untuk melihat profil ini.</EmptyText>
        )}
      </AccordionRow>
  ...
  return (
    <div className={cn(open && tinted && 'bg-[#F7F6FD]')}>
      <button type="button" onClick={onToggle} aria-expanded={open} className="flex w-full items-center justify-between gap-3 p-4 text-left">
        <div>
          <h2 className={cn('font-heading text-[18px] leading-[27px] font-bold text-[#20202A]', titleClassName)}>{title}</h2>
          <p className={cn('mt-0.5 text-sm text-[#8A8A98]', subtitleClassName)}>{subtitle}</p>
        </div>
        {open ? <ChevronDown className="size-5 shrink-0 text-[#8A82D8]" /> : <ChevronRight className="size-5 shrink-0 text-[#B9B9C6]" />}
      </button>
  ...
  return (
    <>
      <div className="flex items-center gap-3">
        ...
        <span className="rounded-lg bg-[#EFEEFF] px-2.5 py-1 text-sm font-bold tracking-[0.15em] text-[#4138D8]">{riasec.dominant_code}</span>
      </div>
      <div className="mt-3">
        <BarChartSvg
...
function BarChartSvg({ bars, accent = '#3D28C9', muted = '#C9BEF3' }: { bars: Array<{ key: string; label: string; value: number }>; accent?: string; muted?: string }) {
  const width = 560;
  const height = 165;
  const padTop = 18;
  const padBottom = 20;
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Grafik skor" className="w-full overflow-visible">
      {[0.25, 0.5, 0.75, 1].map((fraction) => {
        const y = padTop + areaHeight * (1 - fraction);
        return <line key={fraction} x1={0} x2={width} y1={y} y2={y} stroke="#E4DEF7" strokeDasharray="3 5" />;
      })}
      {bars.map((bar, index) => {
        ...
        return (
          <g key={bar.key}>
            <text x={x + barWidth / 2} y={y - 8} textAnchor="middle" fontSize="13" fontWeight="700" fill="#3A3A48">
              {Math.round(bar.value)}
            </text>
            <rect x={x} y={y} width={barWidth} height={barHeight} rx="8" fill={isMax ? accent : muted} />
            <text x={x + barWidth / 2} y={height - 6} textAnchor="middle" fontSize="12" fill="#7C7C8C">
              {bar.label}
            </text>
          </g>
        );
...
function SaktiInsightsPanel({ assessment, career, targetRole }: { assessment: AssessmentResultResponse | null; career: DoubleDiamondResultResponse | null; targetRole: string | null }) {
  ...
  if (!assessment && !career) {
    return (
      <section className="rounded-lg bg-white p-5 ring-1 ring-[#ECECF2]">
        ...
        <p className="mt-4 rounded-lg bg-[#FAFAFC] p-5 text-center text-sm leading-6 text-[#8A8A98]">Insight tersedia setelah kamu menyelesaikan asesmen psikometri dan eksplorasi karier.</p>
      </section>
    );
  }

  const peakRole = targetRole || career?.recommended_roles?.[0]?.label || null;
  const competency = assessment?.ocean ? competencyScores(assessment.ocean, assessment.riasec) : null;
  ...
  const renderContent = (clamped = false) => (
    <>
      ...
      {narrative ? (
        ...
        <EmptyText>Ringkasan naratif tersedia setelah eksplorasi karier selesai.</EmptyText>
      )}
      ...
      {competency && (
        <div className="shrink-0">
          ...
          <div className="mt-1.5 grid gap-2 sm:grid-cols-3">
            ...
            <ScoreTile tone="amber" icon={Users} value={competency.people} label="People-related" description="Komunikasi, kolaborasi, dan pengaruh sosial" />
            <ScoreTile tone="emerald" icon={ShieldCheck} value={competency.self} label="Self-related" description="Ketahanan stres, adaptabilitas, dan disiplin kerja" />
          </div>
        </div>
      )}
      ...
      <div className="grid gap-3 sm:grid-cols-2 pt-4 shrink-0 relative z-10" onClick={(e) => e.stopPropagation()}>
        ...
        <Link
          ...
          className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#E4E3F0] px-4 py-2 text-[13px] font-semibold text-[#4138D8] transition hover:border-[#CFC8FF] hover:bg-[#F6F5FF]"
        >
          <Briefcase className="size-4" />
          Cari Pekerjaan
        </Link>
...
function ScoreTile({ value, label, description, tone, icon: Icon }: { value: number; label: string; description: string; tone: 'violet' | 'amber' | 'emerald'; icon: typeof Target }) {
  const tones = {
...
function EmptyText({ children }: { children: React.ReactNode }) {
  return <p className="rounded-lg bg-[#FAFAFC] p-4 text-center text-sm leading-6 text-[#8A8A98]">{children}</p>;
}
          ...
          {roles.map((role) => {
            ...
            return (
              <div key={role.code} className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate text-[17px] text-[#26262F]">{role.label}</p>
                  <p className="mt-0.5 truncate text-xs text-[#8A8A98]">{companies.length > 0 ? companies.join(' · ') : role.reason}</p>
                </div>
...
function ProfileCompletionSection({ session }: { session: OnboardingSessionResponse | null }) {
    ...
    <section className="p-4">
            ...
            >
              {isActive ? (
                <span className="flex items-center gap-1.5 rounded-lg bg-[#EFEEFF] px-2.5 py-1.5 text-xs font-semibold text-[#4138D8]">
                  <span className="flex size-4 items-center justify-center rounded-full bg-[#4138D8] text-[10px] font-bold text-white">{index + 1}</span>
                  {step.title}
                </span>
              ) : (
                <span className={cn('flex size-7 shrink-0 items-center justify-center rounded-full text-[11px] font-bold', isDone ? 'bg-[#4138D8] text-white' : 'bg-[#EEEEF3] text-[#9A9AAB]')} title={step.title}>
      ...
      {!completed && activeStep && (
        <div className="mt-3 flex items-start gap-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">
          ...
          <span>
            Kamu masih belum menyelesaikan <span className="font-semibold">{activeStep.title}</span>
          </span>
        </div>
      )}

      <p className="mt-2.5 text-sm leading-6 text-[#6C6C7A]">
        {completed ? 'Semua tahap onboarding sudah selesai. Profilmu siap untuk proses matching.' : activeStep?.description}
        {!completed && (
          <Link href="/job-seeker/onboarding" className="ml-1 font-semibold text-[#4138D8] hover:underline">
            Selesaikan →
          </Link>
        )}
      </p>

      <p className="mt-3 text-xs font-medium text-[#9A9AAB]">atau jelajahi fitur lainnya</p>
...
function topMatchSkillGaps(jobMatcher: JobMatcherRun) {
  ...
  const ranked = [...jobMatcher.result.career_match_results].sort((left, right) => right.total_match_score - left.total_match_score);
  const topMatchId = ranked[0]?.match_id;
  if (!topMatchId) return [];
  return jobMatcher.result.skill_gap_results.filter((gap) => gap.match_id === topMatchId);
}
...
function onboardingStage(session: OnboardingSessionResponse | null): number {
  ...
  if (session.current_step === 'DIVERGE_1' || session.current_step === 'CONVERGE_1' || session.current_step === 'DIVERGE_2' || session.current_step === 'CONVERGE_2') {
    return 2;
  }
  return 3;
}
...
function competencyScores(ocean: Ocean, riasec: Riasec) {
  ...
  return {
    task: clamp((ocean.conscientiousness + riasec.investigative + riasec.conventional) / 3),
    people: clamp((ocean.extraversion + ocean.agreeableness + riasec.social + riasec.enterprising) / 4),
